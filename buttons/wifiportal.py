...
def captive_portal(essid_name, dhcp_name):
	...
	try:
		...
		print(j)
		f.close()
		con = connection(j['network_name'], j['network_password'], dhcp_name)
		if con is True:
		...
		while set_connection is False:
			...
			try:
				...
				if search_result:
					...
					print("Got connection: %s %s %s" %( incoming_network_name, incoming_network_pass, dhcp_name))
					con = connection(incoming_network_name, incoming_network_pass, dhcp_name)
					if con is True:
